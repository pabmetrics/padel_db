...
def build(sexo: str = "M") -> dict:
    """Genera las 3 variantes del gráfico y devuelve los metadatos del
    candidato (doc 03 §6: registro, serie, values, fuente_txt, rutas a los
    PNG) para que `copy_factory` pueda escribir el texto sin tener que
    recalcular qué dato es el titular — `values` es exactamente el dato
    que ya destaca el título del propio gráfico."""
