...
**Fase actual: Fase 2 — Lo que nadie tiene** (sección 8 de `padel-datos-01-arquitectura.md`), en curso desde el 17/09/2026. Fases 0 y 1 cerradas. De Fase 2 ya hay `dim_pareja` (SCD2) y tres gold nuevas (`parejas_duracion`, `h2h`, `torneo_sorpresas`), construidas sin necesitar nada del usuario. Falta `puntos_a_defender` y `ganancias_temporada`, que si necesitan que el usuario aporte los reglamentos de puntos/prize money en PDF.
...
| Gold: `puntos_a_defender` / `ganancias_temporada` | Pendiente | Necesita `data/manual/puntos_prize_2026.csv` (reglamentos FIP/Premier Padel en PDF) — el usuario tiene que aportarlos o confirmar la fuente |
...
5. **`data/manual/puntos_prize_2026.csv`, `dim_puntos_categoria`, `fact_resultado_torneo`, gold `puntos_a_defender` y `ganancias_temporada`** — pendiente. Necesita los reglamentos de FIP/Premier Padel en PDF (tabla de puntos por ronda y categoría, y reparto de prize money); es curación manual que solo puede aportar o confirmar una persona.
