...
**Fase actual: Fase 1 — Núcleo del circuito** (sección 8 de `padel-datos-01-arquitectura.md`), **cerrada** el 17/09/2026. Identidad y presencia resueltas; Fase 0 cerrada; Fase 1 tiene `dim_jugador`, `map_jugador_fuente`, `dim_torneo`, `fact_ranking_semanal` definitivo, `fact_partido`, tests de calidad y tres tablas gold reales, más el primer gráfico automático.
...
**Siguiente: Fase 2 — Lo que nadie tiene** (semanas 4-5, doc 01 §8): `data/manual/puntos_prize_2026.csv` desde los reglamentos (PDF, curación manual — necesita que el usuario aporte o confirme la fuente); `dim_puntos_categoria`; `fact_resultado_torneo`; gold `puntos_a_defender` y `ganancias_temporada`; derivar `dim_pareja` (SCD2) desde `fact_partido` (ya disponible) y gold `parejas_duracion`, `h2h`, `torneo_sorpresas`, `torneo_previa`; unificar `dim_torneo` (F1) con los `torneo_id` de `fact_partido` (F2) vía `event_code`; etiquetado manual del lado de pista solo para quien F2 no lo trae (la mayoría del top 100 ya lo tiene).
