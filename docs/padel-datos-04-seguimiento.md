...
| `fact_ranking_semanal` definitivo | Hecho | `transform/build_fact_ranking_semanal.py`: posición de F1 + puntos y delta semanal de F2, unidos por `jugador_id` (no por nombre). 795/1.000 filas con puntos de F2 |
...
8. Conector F3 (padelfip.com, scraper) — no empezado, todavía no ha hecho falta porque F1+F2 ya cubren ranking y calendario.
